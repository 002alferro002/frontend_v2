                    ...
                    if response.status == 200:
                        ...
                        if data.get('retCode') == 0:
                            ...
                            if expected_range_min <= exchange_time <= expected_range_max:
                                logger.info(
                                    f"✅ Время синхронизировано с биржей Bybit. Смещение: {self.time_offset:.0f}мс, задержка сети: {network_delay:.0f}мс")
                                logger.info(f"🕐 Биржевое время: {exchange_time}, локальное: {current_unix_ms:.0f}")
                                return True
                            else:
                                logger.error(
                                    f"❌ Некорректное время биржи: {exchange_time} (ожидается в диапазоне {expected_range_min}-{expected_range_max})")
                                self.is_synced = False
                                return False
                        else:
